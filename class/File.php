...
class File{
    ...
    public function CheckBackground(){
        ...
        if(!empty($this->file['background']['name']))
            {

            if(isset($this->file["background"]) && $this->file["background"]["error"] == 0){
                $extensions_list = array('.jpg');
                $extension = strrchr($this->file['background']['name'], '.');

                    if($this->file['background']['size'] < 2048000){
    ...
    /**
        * Background import
        */
    public function ImportBackground(){

        if(file_exists('uploads/background.jpg')){
            
            unlink('uploads/background.jpg');
        
            move_uploaded_file($this->file["background"]["tmp_name"], 'uploads/background.jpg');
    ...
    /**
        ...
        * @param string directory link
        */
    public function DirectoryReset($dir){
